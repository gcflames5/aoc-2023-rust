...
fn part1() {
    ...
    let points: u32 = contents
        .lines()
        .map(|line| Line::from_string(line).get_score())
        .sum();
...
fn part2() {
    ...
    let card_id_to_matches_map: HashMap<u8, u32> = contents
        .lines()
        .map(|line_str| {
    ...
    let mut card_id_to_copies_map: HashMap<u8, u32> = card_id_to_matches_map
        .iter()
        .map(|entry| (*entry.0, 1))
        .collect();
    ...
    for card_id in 1..(num_ids + 1) {
        ...
        for copy_card_id in (card_id + 1)..(card_id + 1 + usize::try_from(num_matches).unwrap()) {
            *card_id_to_copies_map
                .get_mut(&u8::try_from(copy_card_id).unwrap())
                .unwrap() += num_copies;
        }
    }
